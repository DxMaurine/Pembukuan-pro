...
import { X, Package, Trash2, FilePlus, CreditCard, Plus, Info } from 'lucide-react';
        ...
        <form onSubmit={handleAddTransaction} className="space-y-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 bg-slate-100 dark:bg-white/5 p-3 rounded-2xl border border-slate-200/50 dark:border-white/5 shadow-inner">
            <div className="flex-1 flex flex-col gap-1">
              <span className="text-[9px] uppercase font-black tracking-widest text-muted opacity-60">Tanggal Transaksi:</span>
              <input
                type="date"
            ...
            <div className="flex-1 flex flex-col gap-1">
              <span className="text-[9px] uppercase font-black tracking-widest text-muted opacity-60">Arus Kas (Tipe):</span>
              <select
              ...
              >
                <option value="income">PELAYANAN / PEMASUKAN (+)</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block mb-1 text-[10px] font-black uppercase tracking-widest text-muted">Total Jumlah (Rp)</label>
                <input
                  type="text"
                  className={`form-input text-2xl font-black text-center py-3 shadow-lg shadow-black/5 bg-slate-50 dark:bg-white/5 border-2 ${
              ...
              <div className="space-y-3">
                <div>
                  <label className="block mb-1 text-[10px] font-black uppercase tracking-widest text-muted">Keterangan Utama</label>
                  <input
                ...
                <div>
                  <label className="block mb-1 text-[10px] font-black uppercase tracking-widest text-muted">Kategori</label>
                  <input
                    className="form-input py-2.5 text-sm font-medium"
              ...
              <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10">
                <span className="text-[9px] uppercase font-black tracking-widest text-muted block mb-2">Ringkasan Hari Ini:</span>
                <div className="flex justify-between items-center text-xs mb-1">
                  <span className="font-medium opacity-70">Pemasukan:</span>
                  <span className="text-success font-black">Rp {formatIDR(isIncome ? formData.amount : 0)}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="font-medium opacity-70">Pengeluaran:</span>
                  <span className="text-primary font-black">Rp {formatIDR(isIncome ? 0 : formData.amount)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col h-full">
              <div className="flex justify-between items-center mb-3">
                <div className="flex flex-col">
                  <label className="text-xs font-black uppercase tracking-wide">Rincian Detail</label>
                  <span className="text-[8px] text-muted font-bold uppercase tracking-[0.2em] opacity-50">Sub-Items Breakdown</span>
                </div>
                <button
                  type="button"
                  className={`btn py-1.5 px-3 text-[9px] font-black uppercase border-none shadow-md ${
                    isIncome ? 'btn-success shadow-success/10' : 'btn-primary shadow-primary/10'
                  }`}
              ...
              <div className="flex-1 overflow-y-auto pr-1.5 custom-scrollbar space-y-2 min-h-[220px] max-h-[280px]">
                {formData.items.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center opacity-30 py-8 border-2 border-dashed border-slate-300 dark:border-white/10 rounded-2xl group hover:border-primary/30 transition-colors">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted text-center leading-relaxed">
                  ...
                  formData.items.map((item: any, idx: number) => (
                    <div key={idx} className="flex gap-2 items-center animate-fade-in group">
                      ...
                      <div className="w-[100px] shrink-0">
                        <input
                          className="form-input py-2 px-3 text-[11px] text-right font-black"
                          placeholder="0"
                      ...
                      <button
                        type="button"
                        className="btn btn-danger p-2 shrink-0 opacity-0 group-hover:opacity-100 transition-all hover:scale-110 active:scale-95"
          ...
          <div className="flex flex-col md:flex-row gap-3 pt-4 border-t border-slate-200 dark:border-white/5">
            <button
              type="button"
              className="btn flex-1 justify-center py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all hover:bg-slate-200 dark:hover:bg-white/10"
              onClick={closeModal}
            >
              Kembali
            </button>
            <button
              type="submit"
              className={`btn flex-2 justify-center py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all hover:scale-[1.02] active:scale-95 ${
