    ...
    <div className="flex flex-col gap-8 animate-fade-in">
      ...
      <div className="flex flex-col gap-4">
        {filteredDebts.length === 0 ? (
          <div className="glass-card text-center py-20 opacity-50">Tidak ada catatan hutang ditemukan.</div>
        ) : (
      ...
      {/* Modal Tambah Catatan - Menggunakan Portal agar bisa Full Screen & Center sempurna sesuai SS 2 */}
      {showModal && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center z-[110] px-4 overflow-y-auto">
          <div className="w-full flex justify-center items-center">
            <form onSubmit={handleAddDebt} className="glass-card w-full max-w-lg relative p-8 animate-scale-up space-y-6 border-t-8 border-t-transparent transition-all overflow-hidden" 
              ...
              <div className="flex flex-wrap items-center gap-3 mb-2 pr-12">
                ...
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${formData.type === 'receivable' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>
                  {formData.type === 'receivable' ? 'Piutang' : 'Hutang'}
                </span>
              </div>
              ...
              <div className="flex bg-slate-100 dark:bg-bg-dark/40 p-1.5 rounded-xl border border-slate-200/50 dark:border-border/50">
                <button 
                  ...
                  className={`flex-1 py-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${formData.type === 'receivable' ? 'bg-white text-emerald-600 shadow-md scale-[1.02]' : 'text-muted'}`}
                >
                  PIUTANG (Plg Hutang)
                </button>
                <button 
                  ...
                  className={`flex-1 py-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${formData.type === 'payable' ? 'bg-white text-rose-600 shadow-md scale-[1.02]' : 'text-muted'}`}
                >
                  HUTANG (Toko Hutang)
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div className="group">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted block mb-1 flex items-center gap-2">
                    <Search size={12} className={formData.type === 'receivable' ? 'text-emerald-500' : 'text-rose-500'} /> 
                    Nama Pihak / Pelanggan:
                  </label>
                  <input 
                    required 
                    className={`form-input focus:ring-2 ${formData.type === 'receivable' ? 'focus:border-emerald-500 focus:ring-emerald-500/20' : 'focus:border-rose-500 focus:ring-rose-500/20'}`} 
                    value={formData.name} 
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted block mb-1 flex items-center gap-2">
                    <Plus size={12} className={formData.type === 'receivable' ? 'text-emerald-500' : 'text-rose-500'} />
                    Nominal (Rp):
                  </label>
                  <input 
                    required 
                    className={`form-input text-xl font-bold ${formData.type === 'receivable' ? 'text-emerald-600 focus:border-emerald-500 focus:ring-emerald-500/20' : 'text-rose-600 focus:border-rose-500 focus:ring-rose-500/20'}`} 
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted block mb-1 flex items-center gap-2">
                    <AlertCircle size={12} className={formData.type === 'receivable' ? 'text-emerald-500' : 'text-rose-500'} />
                    Keterangan (Opsional):
                  </label>
                  <textarea 
                    className={`form-input focus:ring-2 ${formData.type === 'receivable' ? 'focus:border-emerald-500 focus:ring-emerald-500/20' : 'focus:border-rose-500 focus:ring-rose-500/20'}`} 
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted block mb-1 flex items-center gap-2">
                    <CheckCircle size={12} className={formData.type === 'receivable' ? 'text-emerald-500' : 'text-rose-500'} />
                    Tanggal Transaksi:
                  </label>
                  <input 
                    type="date" 
              ...
              <div className="flex gap-4 pt-4">
                <button 
                  type="button" 
                  onClick={() => setShowModal(false)} 
                  className="btn flex-1 justify-center py-4 rounded-2xl font-bold bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10"
                >
                  Batal
                </button>
                <button 
                  type="submit" 
                  className={`btn flex-1 justify-center py-4 rounded-2xl font-bold shadow-lg transition-all ${
