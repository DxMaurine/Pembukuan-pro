...
const PriceManager: React.FC<PriceManagerProps> = ({ prices, customers, loadData, api }) => {
  ...
  const handleAddPrice = async (shouldNotify: boolean = false) => {
    ...
    try {
      ...
      const newPrice = await api.addPrice(payload);
      ...
      Swal.fire('Error', 'Gagal menyimpan harga.', 'error');
    }
  };
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      loadData();
      Swal.fire({ title: 'Berhasil!', text: 'Pelanggan baru ditambahkan.', icon: 'success', timer: 1500, showConfirmButton: false });
    } catch {
  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in pb-10">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex bg-slate-100 dark:bg-bg-dark/40 p-1.5 rounded-2xl border border-slate-200/50 dark:border-border/50">
            <button
              ...
              className={`flex items-center gap-3 px-6 py-3 rounded-xl text-sm font-black transition-all ${activeSubTab === 'prices' ? 'bg-white dark:bg-primary shadow-xl scale-105' : 'text-muted'}`}
            >
              <TrendingUp size={18} /> UPDATE HARGA
            </button>
            <button
              onClick={() => { setActiveSubTab('customers'); setCurrentPage(1); }}
              className={`flex items-center gap-3 px-6 py-3 rounded-xl text-sm font-black transition-all ${activeSubTab === 'customers' ? 'bg-white dark:bg-primary shadow-xl scale-105' : 'text-muted'}`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`glass-card p-5 flex flex-col justify-between border-l-4 group transition-all duration-300 h-[170px] ${activeSubTab === 'prices' ? 'border-primary ring-2 ring-primary/10 bg-gradient-to-br from-primary/[0.05]' : 'border-slate-300 opacity-40 grayscale'}`}>
           <div className="flex justify-between items-start">
             ...
             <span className="text-[10px] font-black text-muted tracking-widest uppercase opacity-60">TOTAL UPDATE HARGA</span>
           </div>
           <div>
             <h3 className="text-2xl font-black italic tracking-tighter text-slate-800 dark:text-white leading-none">{totalPrices} Entri</h3>
        ...
        <div className={`glass-card p-5 flex flex-col justify-between border-l-4 group transition-all duration-300 h-[170px] ${activeSubTab === 'customers' ? 'border-amber-500 ring-2 ring-amber-500/10 bg-gradient-to-br from-amber-500/[0.05]' : 'border-slate-300 opacity-40 grayscale'}`}>
           <div className="flex justify-between items-start">
             ...
             <span className="text-[10px] font-black text-muted tracking-widest uppercase opacity-60">REGISTRY PELANGGAN</span>
           </div>
           <div>
             <h3 className="text-2xl font-black italic tracking-tighter text-amber-600 dark:text-amber-400 leading-none">{totalCustomers} Pelanggan</h3>
        ...
        <div className="glass-card p-5 bg-gradient-to-br from-primary/10 via-transparent to-primary/5 border-none flex flex-col justify-center h-[170px]">
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <span className="font-black uppercase tracking-[0.15em] text-[10px] mb-0.5">Tambah {activeSubTab === 'prices' ? 'Update Harga' : 'Pelanggan Baru'}</span>
                      ...
                      <div key={item.id} className="grid grid-cols-[1.5fr_1.5fr_1.2fr] items-center p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 hover:scale-[1.01] transition-all group shadow-sm">
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-sm font-black uppercase truncate tracking-tight">{item.itemName}</div>
                        ...
                        <div className="flex items-center gap-4">
                           <div className="flex flex-col text-right pr-4 border-r border-slate-200 dark:border-white/10 min-w-[100px]">
                              <span className="text-[9px] font-bold text-muted uppercase opacity-40">Lama:</span>
                              <span className="text-xs font-bold opacity-60">Rp {formatIDR(item.oldPrice)}</span>
                           </div>
                           <div className="flex flex-col min-w-[100px]">
                              <span className="text-[9px] font-bold text-primary uppercase">Baru:</span>
                              <span className="text-sm font-black text-primary italic">Rp {formatIDR(item.newPrice)}</span>
                           </div>
                        </div>
                    ...
                    {currentItems.map(c => (
                      <div key={c.id} className="grid grid-cols-[1.5fr_1fr_1.2fr] items-center p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 hover:scale-[1.01] transition-all group shadow-sm">
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-sm font-black uppercase truncate tracking-tight">{c.name}</div>
                            <div className="text-[9px] text-muted font-bold uppercase tracking-widest">{c.phone || '-'}</div>
                          </div>
                        </div>
                        <div className="flex justify-center">
                           <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-full border ${c.type === 'pelanggan' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-slate-500/10 text-slate-500 border-slate-500/20'}`}>
                             {c.type}
                           </span>
                        </div>
                        <div className="flex items-center justify-end gap-6 group-hover:gap-4 transition-all">
                           <span className="text-[10px] text-muted italic max-w-[150px] truncate">{c.address || 'Tanpa alamat'}</span>
                           <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
      ...
      {showPriceModal && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[200] px-4">
           <div className="glass-card w-full max-w-[600px] p-8 animate-scale-up border-t-8 border-t-primary relative">
              <button onClick={() => setShowPriceModal(false)} className="absolute top-6 right-6 p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                <X size={20} />
              </button>

              <h2 className="text-2xl font-black italic mb-8 flex items-center gap-3">
                <TrendingUp size={28} className="text-primary" /> CATAT UPDATE HARGA
              </h2>
      ...
      {showCustomerModal && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[200] px-4">
           <div className="glass-card w-full max-w-[480px] p-8 animate-scale-up border-t-8 border-t-amber-500 relative">
              <button onClick={() => setShowCustomerModal(false)} className="absolute top-6 right-6 p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                <X size={20} />
              </button>

              <h2 className="text-2xl font-black italic mb-8 flex items-center gap-3 text-amber-500">
                <UserPlus size={28} /> REGISTRY PELANGGAN
              </h2>
              ...
              <form onSubmit={handleAddCustomer} className="space-y-6">
                 ...
                 <button type="submit" className="btn bg-amber-500 hover:bg-amber-600 text-white w-full py-4 shadow-lg shadow-amber-500/20">Simpan Pelanggan</button>
              </form>
           </div>
        </div>,
        document.body
      ...
      <style>{`
        .label-style {
          @apply text-[10px] font-black uppercase tracking-[0.2em] text-muted mb-2 block ml-1 opacity-70;
        }
      `}</style>
    </div>
  );
