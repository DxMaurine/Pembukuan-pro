...
// TODO: Ganti dengan konfigurasi Firebase Anda (Firebase Console > Project Settings > Web Apps)
...
export async function syncQRISToFirebase(entry: any) {
  try {
    ...
    console.log("QRIS Entry synced to Firebase");
  } catch (error) {
...
export async function updateFirebaseQRISStatus(id: string, status: string) {
  try {
    ...
    // pakaisetDoc dengan merge: true agar jika dokumen belum ada, otomatis terbuat
    await setDoc(docRef, { 
      status, 
      updatedAt: new Date().toISOString() 
    }, { merge: true });
...
// --- FITUR AUTO-DANA VIA FIRESTORE (Bisa dari internet, bukan hanya WiFi lokal) ---
export async function pushDanaIncoming(text: string) {
  try {
    const id = Date.now().toString();
    await setDoc(doc(db, "auto_dana_incoming", id), {
      text,
      processed: false,
      createdAt: new Date().toISOString()
    });
    console.log('[FIREBASE] Dana notification pushed:', text);
  } catch (error) {
    console.error('[FIREBASE] Push Dana Error:', error);
  }
}
    ...
    snapshot.docChanges().forEach(async (change) => {
      if (change.type === "added") {
        ...
        if (!data.processed) {
          ...
          // Tandai sudah diproses agar tidak double
